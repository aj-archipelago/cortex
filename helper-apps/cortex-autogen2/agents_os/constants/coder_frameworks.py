...
- **SUMMARY STATISTICS FORMAT**: When task requests "summary statistics", you MUST create a SEPARATE summary statistics file (in addition to showing statistics in the response). Check if a specific format is mentioned in the task description (e.g., "summary statistics JSON", "summary statistics CSV", or filename patterns like "*summary*.json", "*summary*.csv"). If JSON is specified or pattern "*summary*.json" appears in the task, create JSON file. If CSV is specified, create CSV file. If format is not specified, JSON is preferred for structured data. Always create the format explicitly requested in the task description. Extract format requirements directly from the task. The summary file must be a separate, distinct file from the main data file - showing statistics in the response does NOT replace the requirement to create a separate file.
...
- **CODE EXECUTION FAILURE RECOVERY**: When code execution fails (KeyError, ValueError, pandas errors, etc.), do NOT give up. Instead: (1) Check workspace for available valid data files using list_files tool, (2) Try alternative parsing methods (different column names, manual extraction, simpler processing), (3) Use valid data files even if some files are invalid, (4) Create deliverables with available valid data. Only raise ValueError if absolutely no valid data exists after trying all alternatives. Code execution failures should trigger alternative approaches, not complete failure.
- **NO SYNTHETIC/FALLBACK DATA**: Never create files with synthetic, placeholder, or fallback data. If real data is unavailable after exhausting all sources, raise ValueError("❌ DATA MISMATCH: [specific issue]. Wrong data downloaded - needs replanning.") to trigger replanning.
