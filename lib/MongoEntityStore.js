 ...
 * - baseModel: String (optional - base model for the entity, e.g., "gemini-flash-35-vision")
...
export class MongoEntityStore {
     ...
     * @param {boolean|Function} [mongoConfig.ensureIndexes] - Create indexes from this runtime process
     */
    constructor(mongoConfig = {}) {
        ...
        const ensureIndexes = mongoConfig.ensureIndexes ?? shouldEnsureRuntimeIndexes;
        ...
        // Bounded cache for lazily loaded entity definitions
        this._entityCache = new Map();
        const cacheSet = this._entityCache.set.bind(this._entityCache);
        this._entityCache.set = (key, value) => {
            this._entityCache.delete(key);
            cacheSet(key, value);
            while (this._entityCache.size > 512) {
                const oldest = this._entityCache.keys().next().value;
                this._entityCache.delete(oldest);
                this._cacheTimestamps.delete(oldest);
            }
    ...
    async colleagueOutbox() {
        await this._getCollection();
        return this._db.collection(`${this.collectionName}_colleague_outbox`);
    }
    ...
    async getEntity(entityId, options = {}) {
        ...
        if (!fresh && !isStale && this._entityCache.has(entityId)) {
        ...
        try {
            ...
            if (userId) {
                const userFilter = [
                    ...
                    { assocUserIds: { $size: 0 } },
                    { assocUserIds: userId },
                    { kind: 'colleague', assistantVisibility: 'public' },
                    { kind: 'colleague', 'assistantAccess.userId': userId }
                ];
    ...
    /** Atomically replace only the workspace if the caller's snapshot is still current. */
    async compareAndSetWorkspace(entityId, expectedWorkspace, nextWorkspace) {
        if (!entityId || !this.isConfigured()) throw new Error('Workspace persistence is unavailable');
        const collection = await this._getCollection();
        const result = await collection.updateOne(
            { id: entityId, workspace: expectedWorkspace ?? null },
            { $set: { workspace: nextWorkspace, updatedAt: new Date() } },
        );
     ...
     */
    async entityPreferences() {
        await this._getCollection();
        return this._db.collection(`${this.collectionName}_user_preferences`);
    }

    async upsertEntity(entity, { insertOnly = false } = {}) {
        if (!this.isConfigured()) {
        ...
        try {
            ...
            if (!entity?.id && !entity?.name) {
                logger.error('Refusing to create unnamed entity without explicit id');
                return null;
            }
            ...
            const doc = {
                ...
                kind: pick('kind', null),
                colleagueOwnerId: pick('colleagueOwnerId', null),
                colleagueStatus: pick('colleagueStatus', null),
                assistantVisibility: pick('assistantVisibility', 'private'),
                assistantAccess: pick('assistantAccess', []),
                assistantMaterials: pick('assistantMaterials', false),
                ...(pick('kind', null) === 'colleague' ? { assistantMaterialsContext: `applet-shared:${createHash('sha256').update(`assistant-materials:${id}`).digest('hex').slice(0, 24)}` } : {}),
                workspaceOwnerId: pick('workspaceOwnerId', null),
            ...
            // An unrelated edit from a stale instance must not restore revoked
            // access or disable newly attached materials.
            const persistedDoc = { ...doc };
            if (existingEntity) {
                for (const field of ['assistantVisibility', 'assistantAccess', 'assistantMaterials']) {
                    if (!Object.hasOwn(entity, field)) delete persistedDoc[field];
                }
            }

            await collection.updateOne(
                { id },
                insertOnly ? { $setOnInsert: { ...doc, createdAt } } : {
                    $set: persistedDoc,
            ...
            if (insertOnly) {
                ...
                await this.getEntity(id, { fresh: true, throwOnError: true });
                return id;
            }
            // Update cache
            const cachedDoc = {
