...
async function sendStream(url, stream, filename) {
    try {
        ...
        console.log(`Stream sent successfully: ${response.status}`);
    } catch (error) {
        console.error(`Error sending stream: ${error.message}`);
    }
}
...
module.exports = {
    splitMediaFile, deleteTempFolder, processYoutubeUrl, deleteFile, isValidYoutubeUrl
};