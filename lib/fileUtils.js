 ...
 * @param {string} container - Optional container name where the file is stored
 * @returns {Promise<boolean>} True if file was deleted, false if not found or error
 */
async function deleteFileByHash(hash, pathwayResolver = null, container = null) {
    ...
    try {
        ...
        let deleteUrl = `${fileHandlerUrl}${separator}hash=${encodeURIComponent(hash)}`;
        
        // Add container parameter if provided
        if (container) {
            deleteUrl += `&container=${encodeURIComponent(container)}`;
        }
...
async function addFileToCollection(contextId, contextKey, url, gcs, filename, tags = [], notes = '', hash = null, fileUrl = null, pathwayResolver = null, permanent = false) {
    ...
    const containerName = permanent && process.env.CORTEX_MEDIA_PERMANENT_STORE_NAME 
    ...
    if (fileUrl && (!url || (!url.includes('blob.core.windows.net') && !url.includes('storage.googleapis.com')))) {
        ...
        const uploadResult = await uploadFileToCloud(fileUrl, null, filename, pathwayResolver, containerName);
...
async function generateFileMessageContent(fileParam, contextId, contextKey = null) {
    ...
    const fileWithShortLivedUrl = await ensureShortLivedUrl(foundFile, MEDIA_API_URL);
 ...
 */
async function checkHashExists(hash, fileHandlerUrl, pathwayResolver = null, container = null, shortLivedMinutes = 5) {
    ...
    try {
        const separator = fileHandlerUrl.includes('?') ? '&' : '?';
        let checkHashUrl = `${fileHandlerUrl}${separator}hash=${hash}&checkHash=true`;
        
        // Add container parameter if provided
        if (container) {
            checkHashUrl += `&container=${encodeURIComponent(container)}`;
        }
        
        // Request short-lived URL
        if (shortLivedMinutes) {
            checkHashUrl += `&shortLivedMinutes=${shortLivedMinutes}`;
        }
 ...
 */
async function ensureShortLivedUrl(fileObject, fileHandlerUrl, shortLivedMinutes = 5) {
    ...
    try {
        const resolved = await checkHashExists(fileObject.hash, fileHandlerUrl, null, null, shortLivedMinutes);
 ...
 */
async function uploadFileToCloud(fileInput, mimeType = null, filename = null, pathwayResolver = null, containerName = null) {
    ...
    try {
        ...
        if (fileBuffer) {
            ...
            // Check if file already exists using checkHash (with container if specified)
            const existingFile = await checkHashExists(fileHash, fileHandlerUrl, pathwayResolver, containerName);
            if (existingFile) {
                return existingFile;
            }
    ...
    const fileContentPromises = fileHashes.map(async (hash) => {
        try {
            ...
            if (fileHandlerUrl && fileHandlerUrl !== 'null') {
                // Use shared checkHashExists function - it already returns shortLivedUrl in url field
                const existingFile = await checkHashExists(hash, fileHandlerUrl);
                if (existingFile) {
                    // checkHashExists already returns shortLivedUrl (prefers converted) in url field
                    // and GCS URL (prefers converted) in gcs field
                    // We need filename from the checkHash response, so make a direct call
                    try {
                        const separator = fileHandlerUrl.includes('?') ? '&' : '?';
                        const checkHashUrl = `${fileHandlerUrl}${separator}hash=${hash}&checkHash=true&shortLivedMinutes=5`;
                        const response = await axios.get(checkHashUrl, {
                            timeout: 10000,
                            validateStatus: (status) => status >= 200 && status < 500
                        });
                        ...
                        if (response.status === 200 && response.data) {
                            ...
                            return JSON.stringify({
                                type: "image_url",
                                url: data.shortLivedUrl || data.converted?.url || data.url,
                                image_url: { url: data.shortLivedUrl || data.converted?.url || data.url },
                                gcs: data.converted?.gcs || data.gcs || null, // GCS from checkHash (no short-lived)
                                originalFilename: data.filename,
                                hash: hash
                            });
                        }
                    } catch (error) {
                        // Fallback to existingFile data if direct call fails
                    }
                    
                    // Fallback: use data from checkHashExists
                    return JSON.stringify({
                        type: "image_url",
                        url: existingFile.url, // Already has shortLivedUrl
                        image_url: { url: existingFile.url },
                        gcs: existingFile.gcs || null, // GCS from checkHash
