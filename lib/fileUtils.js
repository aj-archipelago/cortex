...
import { cfhAxios as axios } from './cfhClient.js';
 ...
 * @param {string|null} options.userId - Physical user container owner ID
 * @param {string|null} options.appletId - Applet ID for applet routing
...
async function listFileNamesForContext(
    contextId,
    {
        userId = null,
        appletId = null,
    ...
    const fileHandlerUrl = MEDIA_API_URL;
    if (!fileHandlerUrl || fileHandlerUrl === 'null' || !contextId) {
        return { items: [], truncated: false };
    }

    const span = latencyTrace.start('files.listContextNames', {
        contextId,
        userId,
        appletId,
        chatId,
    ...
    try {
        ...
        latencyTrace.end(span, {
            status: response?.status,
            fileCount: response.data?.items?.length || 0,
        ...
        return {
            ...
            metadataIncluded: response.data?.metadataIncluded === true,
            cacheHit: response.data?.cacheHit === true,
        };
    } catch (error) {
        latencyTrace.end(span, { error: error.message });
        logger.warn(`Failed to list file names for context ${contextId}: ${error.message}`);
        return { items: [], truncated: false };
    }
}

async function listFileNamesForAccessTarget(target, options = {}) {
    const maxResults = Math.min(
        Math.max(parseInt(options.maxResults, 10) || 10000, 1),
        50000,
    );
    return listFileNamesForContext(target.contextId, {
...
function normalizeListNamesSubPath(value) {
    ...
    for (const prefix of prefixedForms) {
        if (lowered === prefix.slice(0, -1)) {
            return null;
        }
...
function getListNamesScopePathForTarget(target) {
    return normalizeListNamesSubPath(constructFolderPath({
...
function getListNamesSubPathForTarget(subPath, target) {
    const normalizedSubPath = normalizeListNamesSubPath(subPath);
    if (!normalizedSubPath) return null;

    const scopePath = getListNamesScopePathForTarget(target);
    if (!scopePath) return normalizedSubPath;

    const loweredSubPath = normalizedSubPath.toLowerCase();
    const loweredScopePath = scopePath.toLowerCase();
    if (loweredSubPath === loweredScopePath) {
        return null;
    }
    if (loweredSubPath.startsWith(`${loweredScopePath}/`)) {
        return normalizedSubPath.slice(scopePath.length + 1) || null;
    }
...
const CONTEXT_FILE_REF_PREFIX = 'context-file:';

function createContextFileRef(contextId, blobPath) {
    if (!contextId || !blobPath) return null;
    const payload = Buffer.from(
        JSON.stringify([String(contextId), String(blobPath)]),
        'utf8',
    ).toString('base64url');
    return `${CONTEXT_FILE_REF_PREFIX}${payload}`;
}

function parseContextFileRef(value) {
    if (typeof value !== 'string' || !value.startsWith(CONTEXT_FILE_REF_PREFIX)) {
        return null;
    }
    try {
        const parsed = JSON.parse(Buffer.from(
            value.slice(CONTEXT_FILE_REF_PREFIX.length),
            'base64url',
        ).toString('utf8'));
        if (
        ...
        return { contextId: parsed[0], blobPath: parsed[1] };
    } catch {
        return null;
    }
}
...
async function listFilesForFileAccessPlan(fileAccessPlan) {
    ...
    for (const target of targets) {
        ...
        const files = await listFilesForContext(target.contextId, {
            userId: target.userContextId || null,
            appletId: target.appletId || null,
            chatId: target.chatId || null,
            workspaceId: target.workspaceId || null,
    ...
    const deduped = deduplicateCloudFiles(allFiles);
...
function normalizeListedFileNameTuple(item, target) {
    const tuple = Array.isArray(item)
        ? item
        : [item?.blobPath || item?.name, item?.size ?? null, item?.lastModified || item?.updatedAt || null];
    const blobPath = tuple[0];
    if (!blobPath || typeof blobPath !== 'string') {
        return null;
    }

    return {
        ...(tuple[3] && typeof tuple[3] === 'object' ? tuple[3] : {}),
        name: blobPath,
        blobPath,
        size: tuple[1] ?? null,
        lastModified: tuple[2] || null,
        filename: path.posix.basename(blobPath),
        _contextId: target.contextId,
        _contextKey: target.contextKey || null,
        _contextFileScope: target.fileScope || null,
        _fileAccessKind: target.kind,
...
async function listFileNamesForFileAccessPlan(fileAccessPlan, options = {}) {
    const targets = resolveFileAccessPlanTargets(fileAccessPlan);
    if (targets.length === 0) {
        return { files: [], truncated: false };
    }

    const maxResultsPerTarget = Math.min(
        Math.max(parseInt(options.maxResultsPerTarget, 10) || 10000, 1),
        50000,
    );
    const subPath = typeof options.subPath === 'string' && options.subPath.trim()
        ? options.subPath.trim()
        : null;
    const span = latencyTrace.start('files.listAccessPlanNames', {
        targetCount: targets.length,
        maxResultsPerTarget,
        subPath,
        contextIds: targets.map((target) => target.contextId).join(','),
    ...
    const allFiles = [];
    let truncated = false;
    let metadataIncluded = true;
    let cacheHit = false;

    for (const target of targets) {
        const targetSpan = latencyTrace.start('files.listAccessPlanTargetNames', {
            contextId: target.contextId,
            kind: target.kind,
            readFileScope: target.readFileScope || 'all',
        });
        const result = await listFileNamesForAccessTarget(target, {
            maxResults: maxResultsPerTarget,
            subPath: getListNamesSubPathForTarget(subPath, target),
            fresh: options.fresh === true,
        });
        latencyTrace.end(targetSpan, {
        ...
        metadataIncluded = metadataIncluded && result.metadataIncluded === true;
        cacheHit = cacheHit || result.cacheHit === true;
        for (const item of result.items) {
            const file = normalizeListedFileNameTuple(item, target);
            if (file) {
                allFiles.push(file);
            }
        }
    ...
    return {
        files: deduped.sort((a, b) => getFileTimestampMs(b) - getFileTimestampMs(a)),
        truncated,
        metadataIncluded,
        cacheHit,
    };
...
function extractBlobPathFromFileRef(fileParam) {
    ...
    if (/^(global|chats|applets|applet-shared|media|profile|articles|skills|automations|documents|projects|users)\//i.test(normalized)) {
        return normalized;
    }
...
function targetAllowsBlobPath(target, blobPath, write = false) {
    if (!blobPath || blobPath.includes('\\') || blobPath.split('/').some(part => part === '.' || part === '..')) return false;
    if (write && target.write !== true) return false;
    const scope = write ? target.writeFileScope : target.readFileScope;
    if (write && !scope) return false;
    const folder = constructFolderPath({
        contextId: target.contextId,
    ...
    return folder !== null && (!folder || blobPath.startsWith(`${folder}/`));
}

function deduplicateCloudFiles(files) {
    const byLocation = new Map();
    for (const file of files) {
        const blobPath = file.blobPath || file.name;
        // Equal bytes in different locations are still different files.
        const key = blobPath ? `${file._contextId}|${blobPath}` : file.url;
        const existing = key && byLocation.get(key);
        if (!existing || (file._writeTarget && !existing._writeTarget)) {
            byLocation.set(key || file, file);
        }
    }
    return [...byLocation.values()];
}

async function lookupFileByBlobPath(blobPath, fileHandlerUrl, target, options = {}) {
    if (!blobPath || !fileHandlerUrl || !target || !targetAllowsBlobPath(target, blobPath)) {
        return null;
    }
    ...
    try {
        const lookupUrl = buildFileHandlerUrl(fileHandlerUrl, {
            blobPath,
            ensureBackup: options.ensureBackup !== false,
        ...
        return {
            ...
            _writeTarget: targetAllowsBlobPath(target, blobPath, true),
            _writeFileScope: target.writeFileScope || null,
        };
    } catch (error) {
...
async function findFileInFileAccessPlanDirect(fileParam, fileAccessPlan, options = {}) {
    ...
    const qualifiedRef = parseContextFileRef(trimmed);
    if (qualifiedRef && fileHandlerUrl) {
        const target = plan.find(
            (candidate) => candidate.contextId === qualifiedRef.contextId
                && targetAllowsBlobPath(candidate, qualifiedRef.blobPath),
        );
        if (!target) return null;
        const prefetched = target.files?.find(
            file => (file.blobPath || file.name) === qualifiedRef.blobPath,
        );
        ...
        const found = await lookupFileByBlobPath(
            qualifiedRef.blobPath,
            fileHandlerUrl,
            target,
            options,
        );
    ...
    if (blobPath && fileHandlerUrl) {
        for (const target of plan) {
            const found = await lookupFileByBlobPath(blobPath, fileHandlerUrl, target, options);
            if (found?.url) return found;
        }
        if (!/^https?:\/\//i.test(trimmed) || options.allowDirectUrl === false) return null;
    }

    if (/^https?:\/\//i.test(trimmed) && options.allowDirectUrl !== false) {
        return {
            url: trimmed,
    ...
    if (hash && fileHandlerUrl) {
        for (const target of plan) {
            ...
            if (found?.url) {
                const legacyPath = found.blobPath || extractBlobPathFromManagedUrl(found.url);
                if (legacyPath && !targetAllowsBlobPath(target, legacyPath)) continue;
                return {
                    ...found,
                    blobPath: legacyPath,
                    _contextId: target.contextId || null,
                    _writeTarget: !!legacyPath && targetAllowsBlobPath(target, legacyPath, true),
                    _writeFileScope: target.writeFileScope,
    ...
    const matched = findFileInCollection(trimmed, names.files);
    if (matched) {
        const target = plan.find(candidate => candidate.contextId === matched._contextId
            && candidate.fileScope === matched._contextFileScope);
        const resolved = await lookupFileByBlobPath(matched.blobPath, fileHandlerUrl, target, options);
        return resolved ? { ...matched, ...resolved,
            displayFilename: matched.displayFilename || resolved.displayFilename } : null;
    }
    if (!names.metadataIncluded) {
        const files = await listFilesForFileAccessPlan(fileAccessPlan);
        return findFileInCollection(trimmed, files);
    }
    return null;
}
...
const downloadFile = async (fileUrl, { timeoutMs = 0 } = {}) => {
    const parsedUrl = new URL(fileUrl);
    const extension = path.extname(parsedUrl.pathname).slice(1) || 'bin';
    const localFilePath = path.join(os.tmpdir(), generateUniqueFilename(extension));
    const controller = new AbortController();
    const timer = timeoutMs > 0 ? setTimeout(() => controller.abort(), timeoutMs) : null;
    try {
        const protocol = parsedUrl.protocol === 'https:' ? https : http;
        const response = await new Promise((resolve, reject) => {
            protocol.get(parsedUrl, { signal: controller.signal }, res => {
        ...
        await pipeline(response, fs.createWriteStream(localFilePath), { signal: controller.signal });
...
async function deleteFilesInFileAccessPlan(files, fileAccessPlan) {
    const outcomes = files.map(() => false);
    const targets = resolveFileAccessPlanTargets(fileAccessPlan);
    const groups = new Map();
    for (let index = 0; index < files.length; index++) {
        const file = files[index];
        const blobPath = file?.blobPath || file?.name;
        const target = targets.find(candidate => candidate.contextId === file?._contextId
            && targetAllowsBlobPath(candidate, blobPath || '', true));
        if (!blobPath || !target || !MEDIA_API_URL) continue;
        if (!groups.has(target)) groups.set(target, []);
        groups.get(target).push({ index, blobPath });
    }
    for (const [target, entries] of groups) {
        for (let offset = 0; offset < entries.length; offset += 500) {
            const batch = entries.slice(offset, offset + 500);
            try {
                const response = await axios.delete(buildFileHandlerUrl(MEDIA_API_URL, {
                    contextId: target.contextId, userId: target.userContextId || null,
                    appletId: target.appletId || null, chatId: target.chatId || null,
                    workspaceId: target.workspaceId || null, fileScope: target.writeFileScope,
                }), { data: { blobPaths: batch.map(entry => entry.blobPath) }, timeout: 120000,
                    validateStatus: status => status >= 200 && status < 500 });
                const byPath = new Map((response.data?.results || []).map(result => [result.blobPath, result]));
                for (const entry of batch) outcomes[entry.index] = response.status === 200
                    && byPath.get(entry.blobPath)?.deleted === true;
            } catch (error) {
...
async function deleteFileInFileAccessPlan(file, fileAccessPlan) {
    return (await deleteFilesInFileAccessPlan([file], fileAccessPlan))[0];
}
 ...
 * @param {Object} metadata - Metadata to update (displayFilename, id, mimeType, lastAccessed)
 * @param {string} contextKey - Optional context key for encryption
 ...
 * @param {string|null} options.appletId - Applet ID for applet scope
 * @param {string|null} options.fileScope - File scope override (default: derived from chatId)
 * @returns {Promise<Object>} File entry object with id
 */
async function addFileToCollection(contextId, contextKey, url, filename, hash = null, fileUrl = null, pathwayResolver = null, chatId = null, { workspaceId, appletId = null, fileScope } = {}) {
    ...
    if (fileUrl && (!url || (!url.includes('blob.core.windows.net') && !url.includes('storage.googleapis.com')))) {
        ...
        // uploadFileToCloud stores a new object at the requested cloud location.
    ...
    const blobPath = extractBlobPathFromManagedUrl(finalUrl);
    if (blobPath) {
        // Uploads already carry their display name on the cloud object. Returning
        // a location reference needs no duplicate hash-keyed collection write.
        return {
            id: createContextFileRef(contextId, blobPath),
            blobPath, url: finalUrl, displayFilename: filename, mimeType,
...
function findFileInCollection(fileParam, collection) {
    ...
    const paramBasename = (normalizedParamPath
        ...
        : path.posix.basename(normalizedParam)).toLowerCase();
    ...
    for (const file of collection) {
        ...
        if (normalizedParamPath) {
            ...
            if ((!normalizedParamUrl || /^file:/i.test(trimmed)) &&
                normalizedParamPath.includes('/') && pathKeys.has(normalizedParamPath)) {
                return file;
            }
    ...
    // A caller that supplies a path or URL has selected a particular file.
    // Never replace it with an unrelated file that happens to share a name.
...
async function generateFileMessageContent(fileParam, fileAccessPlan) {
    ...
    return {
        ...
        blobPath: fileWithShortLivedUrl.blobPath,
        _contextId: getFileContextId(foundFile, plan),
        mimeType: fileWithShortLivedUrl.mimeType || fileWithShortLivedUrl.contentType,
...
async function ensureShortLivedUrl(fileObject, fileHandlerUrl, contextId = null, shortLivedMinutes = 5) {
    if (!fileObject || !fileHandlerUrl) {
        // No file or handler - return original object
        return fileObject;
    }

    // A lookup often already returned the requested short-lived URL. Reuse it
    // without another CFH call, but don't substitute a long-lived storage URL.
    const existingUrl = fileObject.converted?.shortLivedUrl || fileObject.shortLivedUrl || fileObject.url;
    if (isSignedUrlFresh(existingUrl, { maxRemainingMs: shortLivedMinutes * 60_000 })) {
        return { ...fileObject, url: existingUrl, gcs: fileObject.converted?.gcs || fileObject.gcs || null };
    }
    ...
    const blobPath = fileObject.blobPath || fileObject.name || extractBlobPathFromManagedUrl(fileObject.url);
    const scopedContext = fileObject._contextId || contextId;
    if (blobPath && !scopedContext) return fileObject;
    if (!blobPath && !fileObject.hash) return fileObject;
    try {
        const checkHashUrl = buildFileHandlerUrl(fileHandlerUrl, {
            ...(blobPath ? { blobPath, fileScope: 'all' } : { hash: fileObject.hash, checkHash: true }),
            shortLivedMinutes,
            ...(scopedContext ? { contextId: scopedContext } : {}),
        });
...
// Renewal is a scoped read. Image metadata alone never grants access, and a
// URL's expiry cannot authorize looking up the same path in another container.
export async function renewManagedImageUrl(image, fileAccessPlan) {
    ...
    if (!url || !blobPath || !image?._contextId || getSignedUrlTiming(url)?.versioned) return null;
    if (extractBlobPathFromManagedUrl(url) !== blobPath) return null;
    const target = resolveFileAccessPlanTargets(fileAccessPlan).find(candidate =>
        candidate.contextId === image._contextId && targetAllowsBlobPath(candidate, blobPath));
    if (!target) return null;
    const file = await lookupFileByBlobPath(blobPath, config.get('whisperMediaApiUrl'), target, {
        ensureBackup: false, includeMetadata: false, timeoutMs: 10_000,
    });
    if (!file?.url) return null;
    const original = new URL(url);
    const renewed = new URL(file.url);
    if (original.origin !== renewed.origin || original.pathname !== renewed.pathname) return null;
    return { ...image, url: file.url, image_url: { ...image.image_url, url: file.url }, gcs: file.gcs || image.gcs };
}

/**
 * Generic function to upload a file to cloud storage
 * Handles both URLs (downloads then uploads) and base64 data
 * Always uploads to the requested location; content hashes are not file identity
    ...
    try {
        ...
        if (typeof fileInput === 'string') {
            ...
            if (fileInput.startsWith('http://') || fileInput.startsWith('https://')) {
                // Stream remote content to a temporary file for upload.
                ...
                // Download without retaining another copy of the file in memory.
        ...
        if (fileBuffer) {
            // Each upload is stored at its requested cloud location. Do not
            // reuse another location merely because the bytes match.
            if (!tempFilePath) {
...
function normalizeImageDataForUpload(imageData) {
    ...
    const dataUrlMatch = trimmedData.match(/^data:[^;]+;base64,(.*)$/s);
    const base64Data = dataUrlMatch ? dataUrlMatch[1] : trimmedData;

    return Buffer.from(base64Data.replace(/\s/g, ''), 'base64');
}

// Helper function to upload base64 image data to cloud storage.
// Convert image base64 to a Buffer first because JPEG base64 commonly starts
// with "/9j/", which the generic uploader otherwise mistakes for a local path.
const uploadImageToCloud = async (base64Data, mimeType, pathwayResolver = null, fileLocation = null, filename = null) => {
    return await uploadFileToCloud(
        normalizeImageDataForUpload(base64Data),
        mimeType,
        filename,
        pathwayResolver,
        fileLocation
