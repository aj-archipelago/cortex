...
async function checkHashExists(hash, fileHandlerUrl, pathwayResolver = null, contextId = null, shortLivedMinutes = 5) {
    ...
    try {
        ...
        if (checkResponse.status === 200 && checkResponse.data && checkResponse.data.url) {
            ...
            // Prefer converted file's shortLivedUrl if available (that's what we want to send to LLMs)
            // Otherwise use converted URL, then original shortLivedUrl, then original URL
            // For GCS, always use the GCS URL from checkHash (prefers converted, no short-lived for GCS)
            const url = data.converted?.shortLivedUrl || data.converted?.url || data.shortLivedUrl || data.url;
            const gcs = data.converted?.gcs || data.gcs || null;
            
            return {
                url: url, // Prefers converted.shortLivedUrl (for LLM processing), then converted.url, then original shortLivedUrl/url
                gcs: gcs, // GCS URL (prefers converted if available, no short-lived version for GCS)
                hash: data.hash || hash,
...
async function ensureShortLivedUrl(fileObject, fileHandlerUrl, contextId = null, shortLivedMinutes = 5) {
    ...
    // Defensive check: if URL is YouTube, return as-is (shouldn't happen, but handle gracefully)
    ...
    try {
        ...
        if (resolved && resolved.url) {
            // Return file object with url replaced by shortLivedUrl (prefers converted.shortLivedUrl if available)
            // GCS URL comes from checkHash (prefers converted, no short-lived version for GCS)
            // Preserve filename from original, but use resolved filename if original doesn't have one
            return {
                ...fileObject,
                url: resolved.url, // Prefers converted.shortLivedUrl (for LLM processing), then converted.url, then original shortLivedUrl/url
                gcs: resolved.gcs || fileObject.gcs || null, // GCS from checkHash (prefers converted if available)
        ...
        try {
            ...
            if (fileHandlerUrl && fileHandlerUrl !== 'null') {
                ...
                if (existingFile) {
                    ...
                    // - converted.shortLivedUrl (preferred) or converted.url or original shortLivedUrl/url in url field
                    // - GCS URL (prefers converted if available) in gcs field
                    // - filename in filename field
                            return JSON.stringify({
                                type: "image_url",
                        url: existingFile.url, // Prefers converted.shortLivedUrl (for LLM processing)
