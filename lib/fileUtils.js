...
/**
 * Combine original filename with extension from URL
 * This preserves the original filename but uses the actual file type from the URL
 * (important when files are converted, e.g. docx -> md)
 * @param {string} originalFilename - Current filename (may be original from chat history or already-processed filename, may be null)
 * @param {string} url - Azure URL
 * @param {string} gcs - Optional GCS URL
 * @returns {string} Filename with original base name but URL extension
 */
function combineFilenameWithUrlExtension(originalFilename, url, gcs = null) {
    // Get extension from URL (prefer GCS if available)
    const urlFilename = extractFilenameFromUrl(url, gcs);
    
    // Handle null urlFilename (no URL provided)
    if (!urlFilename) {
        // If we have an original filename, return it as-is
        if (originalFilename != null && originalFilename !== '') {
            return originalFilename;
        }
        // No filename available at all
        return null;
    }
    
    const urlExtension = urlFilename.includes('.') ? '.' + urlFilename.split('.').pop() : '';
    
    // If we have an original filename, use its base name with URL extension
    if (originalFilename != null && originalFilename !== '') {
        const originalBase = originalFilename.includes('.') 
            ? originalFilename.substring(0, originalFilename.lastIndexOf('.'))
            : originalFilename;
        return originalBase + urlExtension;
    }
    
    // Fallback to URL filename if no original filename
    return urlFilename;
}
    ...
    const collection = await modifyFileCollectionWithLock(contextId, contextKey, (collection) => {
        ...
        for (const file of extractedFiles) {
            ...
            if (!existsByUrl && !existsByGcs && !existsByHash) {
                ...
                // Use original filename but with extension from URL (in case file was converted)
                const fileEntry = {
                    id: `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
                    url: file.url,
                    gcs: file.gcs || null,
                    filename: combineFilenameWithUrlExtension(file.filename, file.url, file.gcs),
                    hash: file.hash || null,
                    type: file.type || 'file',
                    addedDate: new Date().toISOString(),
                    lastAccessed: new Date().toISOString()
                ...
                if (existingFile) {
                    ...
                    // Track if URLs were updated to avoid redundant filename updates
                    let urlsUpdated = false;
                    
                    // Merge URLs if we have new ones (missing URLs)
                    ...
                    // Update URL/GCS and filename if they have changed (existing URLs that differ)
                    if ((file.url && file.url !== existingFile.url) || (file.gcs && file.gcs !== existingFile.gcs)) {
                    ...
                    // Update filename if URLs were updated (to get correct extension from URL)
                    if (urlsUpdated) {
                        existingFile.filename = combineFilenameWithUrlExtension(existingFile.filename, existingFile.url, existingFile.gcs);
                    }
                    
                    // If we have a new original filename from chat history, update the base name but keep URL extension
                    // Only update if the base name (without extension) differs, not just the extension
                    // Also handles the case where existing file has no filename (adds missing filename)
                    if (file.filename) {
                        // Add missing filename if we have one from chat history
                        if (!existingFile.filename) {
                            existingFile.filename = combineFilenameWithUrlExtension(file.filename, existingFile.url, existingFile.gcs);
                        } else {
                            // Update if base names differ (ignore extension differences from URL updates)
                            const existingBaseName = path.parse(existingFile.filename).name;
                            const newBaseName = path.parse(file.filename).name;
