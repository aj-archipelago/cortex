...
/**
 * Load merged file collection from primary and optional alternate context
 * @param {string} contextId - Primary context ID
 * @param {string} contextKey - Optional encryption key
 * @param {string} altContextId - Optional alternate context ID for union
 * @returns {Promise<Array>} Merged file collection
 */
async function loadMergedFileCollection(contextId, contextKey = null, altContextId = null) {
    const primaryCollection = await loadFileCollection(contextId, contextKey, true);
    
    // Tag primary files with their source context
    const collection = primaryCollection.map(f => ({ ...f, _contextId: contextId }));
    ...
    const altCollection = await loadFileCollectionAll(altContextId, contextKey);
    
    // Build set of existing identifiers from primary collection
    ...
    // Add files from alt collection that aren't already in primary, tagged with alt context
    for (const file of altCollection) {
        ...
        if (!isDupe) {
            collection.push({ ...file, _contextId: altContextId });
        }
    }
 ...
 * @returns {Promise<string>} Formatted string of available files
 */
async function getAvailableFiles(chatHistory, contextId, contextKey = null, altContextId = null) {
    if (!contextId) {
        return 'No files available.';
    }
 ...
 * - Files IN collection (contextId + altContextId): update lastAccessed, strip from message (tools can access)
 * - Files NOT in collection: leave in message (model sees directly)
 * 
 * @param {Array} chatHistory - Chat history array
 * @param {string} contextId - Context ID for file collection (used for updates)
 * @param {string|null} contextKey - Optional encryption key
 * @param {string|null} altContextId - Optional alternate context ID for union (read-only)
 * @returns {Promise<{chatHistory: Array, availableFiles: string}>}
 */
async function syncAndStripFilesFromChatHistory(chatHistory, contextId, contextKey = null, altContextId = null) {
    if (!chatHistory || !Array.isArray(chatHistory)) {
        return { chatHistory: chatHistory || [], availableFiles: 'No files available.' };
    }

    if (!contextId) {
        // No contextId - no collection to check, leave all files in messages
    ...
    // Build lookup map by hash for fast matching and context lookup
    const collectionByHash = new Map(collection.filter(f => f.hash).map(f => [f.hash, f]));
    const collectionByUrl = new Set(collection.map(f => f.url).filter(Boolean));
    const collectionByGcs = new Set(collection.map(f => f.gcs).filter(Boolean));
    ...
    const isInCollection = (contentObj) => {
        ...
        return (fileHash && collectionByHash.has(fileHash)) ||
               (fileUrl && collectionByUrl.has(fileUrl)) ||
               (fileGcs && collectionByGcs.has(fileGcs));
    };

    // Helper to get hash from content object
    const getHash = (contentObj) => contentObj.hash || null;
    
    // Helper to update file when stripped - use _contextId from collection to know which context to update
    const updateStrippedFile = (hash) => {
        if (!hash) return;
        
        const file = collectionByHash.get(hash);
            ...
            const newContent = message.content.map(item => {
                ...
                if (contentObj && (contentObj.type === 'image_url' || contentObj.type === 'file')) {
                    if (isInCollection(contentObj)) {
                        ...
                        const hash = getHash(contentObj);
                        updateStrippedFile(hash); // fire and forget
 ...
 * @param {boolean} options.useCache - If false, bypass cache (default: true)
 * @param {string} options.altContextId - Optional alternate context ID for file collection union
...
export async function resolveFileParameter(fileParam, contextId, contextKey = null, options = {}) {
    ...
    const trimmed = fileParam.trim();
    const { preferGcs = false, useCache = true, altContextId = null } = options;
    ...
    try {
        ...
        // Use merged collection if altContextId is provided (for workspace sub-contexts)
        // useCache can be set to false to bypass cache (e.g., after file edits)
        const collection = altContextId 
            ? await loadMergedFileCollection(contextId, contextKey, altContextId)
            : await loadFileCollection(contextId, contextKey, useCache);
