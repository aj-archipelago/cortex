...
/**
 * @deprecated Use ensureFilenameExtension with mimeType instead
...
function combineFilenameWithUrlExtension(originalFilename, url, gcs = null) {
    // Get extension from URL (prefer GCS if available) - matching old behavior
    ...
    if (originalFilename != null && originalFilename !== '') {
        const originalBase = path.parse(originalFilename).name;
        // If URL has no extension, return base name without extension
        return urlExtension ? originalBase + urlExtension : originalBase;
    }
        ...
        for (const file of extractedFiles) {
            ...
            if (!existsByUrl && !existsByGcs && !existsByHash) {
                ...
                if (existingFile) {
                    ...
                    // Update URLs if they have changed (existing URLs that differ)
                    ...
                    // If we have a new filename from chat history, update it
                    // Ensure it has the correct extension based on stored MIME type
                    if (file.filename) {
                        if (!existingFile.filename) {
                            // Add missing filename
                            existingFile.filename = ensureFilenameExtension(file.filename, existingFile.mimeType);
                        } else {
                            // Update if base names differ (preserve user's preferred name)
                            ...
                            if (existingBaseName !== newBaseName) {
                                // Use new base name but ensure correct extension from MIME type
                                existingFile.filename = ensureFilenameExtension(file.filename, existingFile.mimeType);
                            } else {
                                // Base name matches, just ensure extension is correct
                                existingFile.filename = ensureFilenameExtension(existingFile.filename, existingFile.mimeType);
                            }
                        }
                    } else if (existingFile.filename && existingFile.mimeType) {
                        // No new filename, but ensure existing filename has correct extension
                        existingFile.filename = ensureFilenameExtension(existingFile.filename, existingFile.mimeType);
                    }
...
export { 
    ...
    combineFilenameWithUrlExtension, // Deprecated: use ensureFilenameExtension with mimeType instead
