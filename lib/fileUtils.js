...
async function listFileNamesForFileAccessPlan(fileAccessPlan, options = {}) {
    ...
    const deduped = [];
    for (const file of allFiles) {
        const key = file.blobPath || file.name;
        if (!key || seenBlobPaths.has(key)) {
            continue;
        }
        seenBlobPaths.add(key);
        deduped.push(file);
    }
...
function fileNameSearchHaystack(file) {
    return [
        file?.displayFilename,
        file?.filename,
        file?.name,
...
function fileMatchesNameTerms(file, terms) {
    if (!Array.isArray(terms) || terms.length === 0) return true;
    const haystack = fileNameSearchHaystack(file);
    return terms.every(term => haystack.includes(term));
}

async function findFileByNameViaNameListing(fileParam, fileAccessPlan, options = {}) {
    ...
    const maxResults = Math.min(
        Math.max(parseInt(options.maxResultsPerTarget, 10) || 20000, 1),
        50000,
    );
    const normalizedParamPath = normalizePathForMatch(fileParam);
    const terms = splitSearchTerms(path.posix.basename(normalizedParamPath || fileParam));

    for (const target of targets) {
        const result = await listFileNamesForAccessTarget(target, { maxResults });
        const files = result.items
            .map(item => normalizeListedFileNameTuple(item, target))
            .filter(Boolean);

        const exactPath = normalizedParamPath && files.find((file) => {
            const blobPath = normalizePathForMatch(file.blobPath || file.name);
            return blobPath === normalizedParamPath
                || path.posix.basename(blobPath || '') === path.posix.basename(normalizedParamPath);
        });
        if (exactPath) {
            const resolved = await lookupFileByBlobPath(exactPath.blobPath, options.fileHandlerUrl || MEDIA_API_URL, target);
            if (resolved) return resolved;
        }

        const candidate = pickMostRecentFile(files.filter(file => fileMatchesNameTerms(file, terms)));
        if (candidate?.blobPath) {
            const resolved = await lookupFileByBlobPath(candidate.blobPath, options.fileHandlerUrl || MEDIA_API_URL, target);
            if (resolved) return resolved;
        }
    }
...
async function findFileInFileAccessPlanDirect(fileParam, fileAccessPlan, options = {}) {
    ...
    const nameListed = await findFileByNameViaNameListing(trimmed, plan, {
        ...options,
        fileHandlerUrl,
    });
    if (nameListed) {
