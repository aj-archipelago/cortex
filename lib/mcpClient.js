...
import { createCompanionMcpClient } from './companionMcpClient.js';
...
// Request-local discovery: advertising a configured service must not contact it.
// Retain one attempt per server, including failures, to avoid repeated timeouts.
export function createMcpToolDiscovery(mcpConfigJson) {
    let config = {};
    try {
        config = JSON.parse(mcpConfigJson || '{}');
    } catch {
        logger.warn('Failed to parse mcpConfig for deferred discovery');
    }
    const servers = new Map();
    const expiredServers = [];
    if (config && typeof config === 'object' && !Array.isArray(config)) {
        for (const [key, value] of Object.entries(config)) {
            if (value?.type !== 'local-companion' && (!value?.url || (value.type && value.type !== 'streamable-http'))) continue;
            if (isTokenExpired(value)) expiredServers.push(key);
            else servers.set(key, value);
        }
    }
    ...
    return {
        ...
        serverLabels: Object.fromEntries([...servers].filter(([, value]) => value?.name).map(([key, value]) => [key, String(value.name).replace(/[\r\n]/g, ' ').slice(0, 140)])),
        expiredServers,
        async discover(serverKey) {
            if (!servers.has(serverKey)) throw new Error('Select a configured MCP server to search.');
            if (!attempts.has(serverKey)) {
                attempts.set(serverKey, (async () => {
                    const existing = clients.get(serverKey);
                    const connected = existing ? new Map([[serverKey, existing]])
                        : (await initializeMcpClients(JSON.stringify({ [serverKey]: servers.get(serverKey) }))).clients;
                    if (!connected.size) throw new Error(`MCP server ${serverKey} is unavailable; its tools could not be searched.`);
                    // Keep the shared map so the existing turn/callback cleanup owns these clients.
                    for (const [key, entry] of connected) clients.set(key, entry);
                    // Finish discovery inside the enclosing tool's 120s budget.
                    const discovered = await discoverMcpTools(connected, { timeout: 15000 });
                    if (!Object.keys(discovered.mcpToolCatalog).length) {
                        throw new Error(`MCP server ${serverKey} returned no available tools.`);
                    }
                    return discovered;
                })());
            }
            return attempts.get(serverKey);
        },
    };
}
...
export async function initializeMcpClients(mcpConfigJson) {
    ...
    for (const [serverKey, serverConfig] of Object.entries(config)) {
        if (serverConfig?.type === 'local-companion') {
            try {
                if (!serverConfig.deviceId || !serverConfig.serverId || !serverConfig.token) throw new Error('Invalid local companion configuration');
                clients.set(serverKey, createCompanionMcpClient(serverConfig));
            } catch (error) {
                logger.warn(`Local MCP server ${serverKey} unavailable: ${error.message}`);
            }
            continue;
        }
        if (!serverConfig?.url) {
        ...
        try {
            ...
            await Promise.race([
                ...
                new Promise((_, reject) => {
                    connectionTimer = setTimeout(() => reject(new Error('MCP connection timeout')), MCP_TIMEOUT_MS);
                }),
            ]);
            ...
            // Promise.race does not cancel a pending connect. Abort its requests
            // and SSE/reconnect resources before abandoning this transport.
            try {
                await transport?.close();
            } catch (closeError) {
                logger.warn(`Error closing failed MCP connection ${serverKey}: ${closeError?.message || closeError}`);
            }
 ...
 */
export async function discoverMcpTools(clients, requestOptions) {
    const entityTools = {};
    ...
    for (const [serverKey, { client, connectTimestamp }] of clients) {
        try {
            ...
            if (!entry.cloudId) {
                ...
                if (resourcesTool) {
                    try {
                        ...
                        const resResult = await client.callTool({ name: 'getAccessibleAtlassianResources', arguments: {} }, undefined, requestOptions);
            ...
            for (const tool of tools) {
                ...
                let compositeKey = `${serverKey}__${toolName}`;
                if (entry.localCompanion && compositeKey.length > 64) {
                    const suffix = createHash('sha256').update(tool.name).digest('hex').slice(0, 12);
                    compositeKey = `${serverKey}__${toolName.slice(0, 24)}_${suffix}`;
                }
...
export async function callMcpTool(clients, compositeToolName, args, originalToolName) {
    ...
    logger.info(`[MCP:${serverKey}] calling tool ${toolName} | args=${entry.localCompanion ? "[local tool arguments omitted]" : JSON.stringify(finalArgs).slice(0, 200)} | transport=${transportState} | age=${Date.now() - (connectTimestamp || 0)}ms`);
    ...
    try {
        ...
        logger.info(`[MCP:${serverKey}] tool ${toolName} returned in ${Date.now() - callStart}ms | contentTypes=${content.map(c => c.type).join(',')} | isError=${isError} | resultText=${entry.localCompanion ? "[local tool result omitted]" : text.slice(0, 500)}`);
