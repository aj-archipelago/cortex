...
const { PATHWAY_PUBLISH_KEY, REDIS_CONNECTION_STRING } = process.env;
const PATHWAYS_UPDATES_CHANNEL = 'pathways-updates';
...
class AzureBlobStorage extends StorageStrategy {
  constructor(connectionString, containerName) {
    ...
    this.redisChannel = PATHWAYS_UPDATES_CHANNEL;
  }
...
class PathwayManager {
  constructor(config) {
    ...
    } else if (!REDIS_CONNECTION_STRING) {
      throw new Error('Redis connection string is required for non-local storage types');
    } else {
      this.redis = new Redis(REDIS_CONNECTION_STRING);
      this.redisChannel = PATHWAYS_UPDATES_CHANNEL;
    }
  ...
  async savePathways(pathways) {
    ...
    if (this.redis) {
      await this.redis.publish(this.redisChannel, 'update');
    }
  }
