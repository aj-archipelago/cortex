...
class PathwayManager {
   ...
   * @param {string} [defaultName] - Default name to use if promptItem is a string.
   * @returns {Prompt} A new Prompt object.
   */
  _createPromptObject(promptItem, systemPrompt) {
    ...
    const promptName = typeof promptItem === 'string' ? null : (promptItem.name || null);
  ...
  async getPathways(pathwayTemplate, promptNames = null) {
    ...
    const pathways = prompt.map((p, index) => {
      ...
      const singlePromptPathway = {
        ...
        prompt: [this._createPromptObject(p, systemPrompt)],
