...
class PathwayManager {
  ...
  async putPathway(name, pathway, userId, secret, displayName) {
    ...
    if (this.pathways[name] && this.pathways[name].userId !== userId) {
      throw new Error('Pathway name is already used by another user. Please choose a different name.');
    }

    if (this.pathways[name] && this.pathways[name].secret !== secret) {
      throw new Error('The secret is invalid for this pathway. Please check the secret and try again.');
    }

    this.pathways[name] = { ...pathway, userId, secret, displayName: displayName || pathway.displayName || name };
    await this.savePathways(this.pathways);
    return name;  // Return the final name used
  }

  async removePathway(name, userId, secret) {
    if (!this.pathways[name]) {
      return;
    }
    if (this.pathways[name].userId !== userId || this.pathways[name].secret !== secret) {
      throw new Error('Invalid userId or secret');
    }
    delete this.pathways[name];
    await this.savePathways(this.pathways);
  }
