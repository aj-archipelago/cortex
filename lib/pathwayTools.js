...
const callTool = async (toolName, args, toolDefinitions, pathwayResolver) => {
    ...
    try {
        ...
        if (toolDef.clientSide === true || toolDef.definition?.clientSide === true) {
            logger.info(`Tool ${toolName} is a client-side tool - returning marker for client execution`);
            
            const toolCallbackData = {
                toolUsed: [toolName],
                clientSideTool: true,
                toolCallbackName: toolName,
                toolCallbackId: `${toolName}_${Date.now()}`,
                toolCallbackMessage: args.userMessage || `Executing ${toolName}...`,
                chatId: args.chatId || "",
                toolArgs: args
            };
            
            // Return the marker as the result
            toolResult = JSON.stringify(toolCallbackData);
            ...
            if (pathwayResolver) {
                ...
                try {
                    logger.info(`Publishing client-side tool marker to requestId: ${requestId}`);
                ...
                pathwayResolver.tool = JSON.stringify(toolCallbackData);
                pathwayResolver.pathwayResultData = toolCallbackData;
            }
