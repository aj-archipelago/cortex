...
import { createPermissionWatcher, permissionFailure } from './toolPermissionReview.js';

const permissionWatchers = new WeakMap();

const DEFAULT_CLIENT_TOOL_TIMEOUT_MS = 300000;
const MIN_CLIENT_TOOL_TIMEOUT_MS = 10000;
const MAX_CLIENT_TOOL_TIMEOUT_MS = 15 * 60 * 1000;
const DEFAULT_CLIENT_TOOL_INITIAL_HEARTBEAT_MS = 15 * 1000;
const BACKGROUND_CLIENT_TOOL_HEARTBEAT_STALE_MS = 90 * 1000;

const getClientToolCallbackTimeoutMs = (toolDef) => {
    const requested = Number(toolDef?.definition?.timeout ?? toolDef?.timeout);
    if (!Number.isFinite(requested) || requested <= 0) {
        return DEFAULT_CLIENT_TOOL_TIMEOUT_MS;
    }
    return Math.min(
        MAX_CLIENT_TOOL_TIMEOUT_MS,
        Math.max(MIN_CLIENT_TOOL_TIMEOUT_MS, requested),
    );
};

// Browser timer throttling depends on tab visibility, not tool execution timeout.
// Keep the first acknowledgement deadline short; once acknowledged, allow the
// browser's one-minute background timer cadence without extending the hard cap.
const getClientToolHeartbeatStaleMs = () => BACKGROUND_CLIENT_TOOL_HEARTBEAT_STALE_MS;

const getClientToolWaitOptions = (toolDef) => ({
    maxTimeoutMs: getClientToolCallbackTimeoutMs(toolDef),
    initialHeartbeatTimeoutMs: DEFAULT_CLIENT_TOOL_INITIAL_HEARTBEAT_MS,
    heartbeatStaleMs: getClientToolHeartbeatStaleMs(toolDef),
    checkEveryMs: 1000,
});
...
const callTool = async (toolName, args, toolDefinitions, pathwayResolver) => {
    ...
    try {
        if (config.get('agentPermissionReview.enabled')) {
            if (!pathwayResolver) {
                return permissionFailure({ decision: 'ask', source: 'context', reason: 'Permission review requires an execution context' });
            }
            let authorize = permissionWatchers.get(pathwayResolver);
            if (!authorize) {
                const settings = config.get('agentPermissionReview');
                authorize = createPermissionWatcher({
                    policy: settings.policy || undefined,
                    timeoutMs: settings.timeoutMs,
                    maxReviews: settings.maxReviews,
                    review: input => callPathway('sys_permission_review', {
                        reviewInput: JSON.stringify(input),
                        model: settings.model,
                        stream: false,
                    }, pathwayResolver),
                    record: entry => logger.info(`Permission review: ${JSON.stringify({ requestId: pathwayResolver.requestId, ...entry })}`),
                });
                permissionWatchers.set(pathwayResolver, authorize);
            }
            const verdict = await authorize({ toolName, toolDef, args, requestArgs: pathwayResolver.args,
                isCanceled: () => Boolean(pathwayResolver.isCanceled?.()),
            });
            if (verdict.decision !== 'allow') {
                latencyTrace.end(span, { permissionDecision: verdict.decision, permissionSource: verdict.source });
                return permissionFailure(verdict);
            }
        }
        ...
        if (toolDef.pathwayName === '_builtin_search_tools') {
            ...
            if (args.server && pathwayResolver?.args?.discoverMcpServerTools) {
                const discovered = await pathwayResolver.args.discoverMcpServerTools(args.server);
                Object.assign(pathwayResolver.args.mcpToolCatalog, discovered.mcpToolCatalog);
                Object.assign(pathwayResolver.args.mcpEntityToolsDeferred, discovered.entityTools);
            }
            const catalog = {
            ...
            const scored = Object.values(catalog).filter(entry => !args.server || entry.server === args.server).map(entry => {
        ...
        if (toolDef.clientSide === true || toolDef.definition?.clientSide === true) {
            ...
            if (pathwayResolver) {
                ...
                const clientResultPromise = waitForClientToolResult(
                    toolCallbackId, requestId, getClientToolWaitOptions(toolDef),
                );
 ...
 */
const sendToolFinish = async (requestId, toolCallId, success, error = null, presentation = {}) => {
    try {
        const toolMessage = {
            type: 'finish',
            callId: toolCallId,
            success: success,
            ...(success && presentation.mediaTask ? { mediaTask: presentation.mediaTask } : {}),
        };
...
export { callPathway, gpt3Encode, gpt3Decode, say, callTool, addCitationsToResolver, sendToolStart, sendToolFinish, withTimeout, extractClientToolImages, getClientToolCallbackTimeoutMs, getClientToolHeartbeatStaleMs, getClientToolWaitOptions };
