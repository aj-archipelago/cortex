        ...
        if (toolDef.clientSide === true || toolDef.definition?.clientSide === true) {
            ...
            if (pathwayResolver) {
                ...
                try {
                    ...
                    // Extract images array if present (e.g., from ReadImageContent tool)
                    // Check if clientResult.data has images field (could be object or already stringified)
                    let extractedImages = [];
                    try {
                        const data = typeof clientResult.data === 'string' 
                            ? JSON.parse(clientResult.data) 
                            : clientResult.data;
                        
                        if (data && Array.isArray(data.images)) {
                            // Use images array directly from client result
                            extractedImages = data.images;
                            logger.debug(`Extracted ${extractedImages.length} image(s) from ${toolName} result`);
                        }
                    } catch (e) {
                        // If parsing fails, continue without extracting images
                        logger.debug(`Could not extract images from ${toolName} result: ${e.message}`);
                    }
