...
if (connectionString) {
    ...
    if (subscriptionClient) {
        ...
        subscriptionClient.on('error', (error) => {
            logger.error(`Redis subscriptionClient error: ${JSON.stringify(error)}`);
        });
        ...
        subscriptionClient.on('connect', () => {
            ...
            channels.forEach(channel => {
                subscriptionClient.subscribe(channel, (error) => {
                    if (error) {
                        logger.error(`Error subscribing to redis channel ${channel}: ${JSON.stringify(error)}`);
                    } else {
        ...
        subscriptionClient.on('message', (channel, message) => {
            ...
            if (channel === requestProgressChannel && redisEncryptionKey) {
                try {
                    ...
                    logger.error(`Error decrypting message: ${JSON.stringify(error)}`);
                }
            }
            ...
            try {
                ...
                logger.error(`Error parsing message: ${JSON.stringify(error)}`);
            }
        ...
        try {
            ...
            if (redisEncryptionKey) {
                try {
                    ...
                    logger.error(`Error encrypting message: ${JSON.stringify(error)}`);
                }
            }
