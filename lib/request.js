...
const MAX_RETRY = 10;
const postRequest = async ({ url, params, headers, cache }, model) => {
    let retry = 0;
    const errors = []
    for (let i = 0; i < MAX_RETRY; i++) {
        try {
            ...
            const axiosConfigObj = { headers, cache };
            if (params.stream) {
                axiosConfigObj.responseType = 'stream';
            }
            return await limiters[model].schedule(() => axios.post(url, params, axiosConfigObj));
        } catch (e) {
