...
const postRequest = async (cortexRequest) => {
    ...
    for (let i = 0; i < MAX_RETRY; i++) {
        ...
        if (streamRequested && model.supportsStreaming) {
            ...
            promises.push(selectedEndpoint.limiter.schedule({expiration: 2 * 60 * 1000},() => postWithMonitor(selectedEndpoint, url, data, axiosConfigObj)));
        } else {
            if (streamRequested) {
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            if (!controller.signal?.aborted) {
                                ...
                                response = await selectedEndpoint.limiter.schedule(() => postWithMonitor(selectedEndpoint, url, data, axiosConfigObj));
