...
const LIMITER_DATASTORE_TIMEOUT_MS = 24 * 60 * 60 * 1000;
const DEFAULT_LIMITER_JOB_EXPIRATION_MS = 11 * 60 * 1000;

let modelEndpoints = {};

const getLimiterErrorMessage = (err) => err?.message || String(err);

const buildLimiterScheduleOptions = (id, options = {}) => {
    const scheduleOptions = {
        ...options,
        expiration: options.expiration ?? DEFAULT_LIMITER_JOB_EXPIRATION_MS,
    };
    if (id) {
...
const createLimiter = (endpoint, name, index) => {
    ...
    if (connection) {
        ...
        // Bottleneck timeout is the Redis datastore TTL, not a request timeout.
        // Keep it comfortably above the longest limiter job expiration.
        limiterOptions.timeout = LIMITER_DATASTORE_TIMEOUT_MS;
    }

    endpoint.limiter = new Bottleneck(limiterOptions);

    endpoint.limiter.on('error', (err) => {
        const errorMessage = getLimiterErrorMessage(err);
        const limiterName = `${cortexId}-${name}-${index}`;
        if (errorMessage.includes('UNKNOWN_CLIENT')) {
            logger.warn(`Limiter client state expired for ${limiterName}; Bottleneck will re-register and retry: ${errorMessage}`);
        } else if (errorMessage.includes('too many results to unpack')) {
            logger.error(`Limiter Redis cleanup failed for ${limiterName}; refusing to recreate limiter to avoid client churn: ${errorMessage}`);
        } else {
            logger.error(`Limiter error for ${limiterName}: ${errorMessage}`);
        }
    });
...
const buildModelEndpoints = (config) => {
    ...
    for (const [oldName, newName] of Object.entries(modelRedirects)) {
        const target = modelEndpoints[resolveModelName(newName, config)];
        if (target) {
...
const resolveModelName = (name, resolverConfig = config) => {
    const redirects = resolverConfig.get('modelRedirects') || {};
    ...
    const groups = resolverConfig.get('modelGroups') || {};
...
const requestWithMonitor = async (endpoint, url, data, axiosConfigObj, traceFields = {}) => {
    ...
    try {
        ...
        throw { code, message: errorMessage, status: finalStatus, statusText, name, responseData, duration, headers: error?.response?.headers || error?.headers };
    }
    let duration;
...
const runWithLimiter = (cortexRequest, endpoint, scheduleOptions, task) => {
    ...
    return endpoint.limiter.schedule(
        buildLimiterScheduleOptions(scheduleOptions?.id, scheduleOptions),
        task,
    );
};
...
const applyBypassTimeout = (cortexRequest, axiosConfigObj) => {
    ...
    if (cortexRequest?.executionPolicy?.timeoutMs) {
        axiosConfigObj.timeout = cortexRequest.executionPolicy.timeoutMs;
    }
...
const makeRequest = async (cortexRequest) => {
    ...
    const policy = cortexRequest.executionPolicy;
    const maxRetry = cortexRequest?.bypassLimiter ? 1 : (policy?.maxAttempts ?? MAX_RETRY);
    for (let i = 0; i < maxRetry; i++) {
        const rootId = cortexRequest?.pathwayResolver?.rootRequestId || cortexRequest.requestId;
        const canceled = () => requestState[rootId]?.canceled || requestState[cortexRequest.requestId]?.canceled;
        ...
        axiosConfigObj.signal = providerAbortController.signal;
        const abortProviderRequest = () => providerAbortController.abort();
        registerAbortRequest(rootId, abortProviderRequest);
        if (rootId !== requestId) registerAbortRequest(requestId, abortProviderRequest);
        const dispatch = (endpointName) => {
            if (canceled() || providerAbortController.signal.aborted) throw cancellationError();
            cortexRequest.beforeDispatch?.();
            return requestWithMonitor(selectedEndpoint, url, data, axiosConfigObj, {
                ...traceBase,
        ...
        if (streamRequested && model.supportsStreaming) {
            ...
            resultPromise = runWithLimiter(
                ...
                () => dispatch(selectedEndpoint?.name || 'default')
            );
        } else {
            if (streamRequested) {
        ...
        try {
            ...
            if (isCancellationError(error) || canceled()) {
            ...
            // A caller with bounded remote work owns its retry policy. In
            // particular, a timeout is not evidence that the worker is idle.
            if (policy && (!policy.retryStatuses.includes(status) || i === maxRetry - 1)) {
                throw error;
            }
            ...
            if (policy) {
                backoffTime = Math.max(0, Math.min(Number.isFinite(backoffTime) ? backoffTime : 1000, 10000));
            }
...
const executeRequest = async (cortexRequest) => {
    try {
        ...
        return { data, duration, ...(cortexRequest.searchCache && config.get('searchCacheEnabled')
        ...
        throw cortexRequest?.executionPolicy && !(error instanceof Error)
...
export {
    ...
    pickGroupMember, MODEL_SAMPLE_STALENESS_MS, runWithLimiter, buildLimiterScheduleOptions
};
