...
// Chat Completions uses { type: 'text' | 'image_url' } content parts; the
// Responses API uses { type: 'input_text' | 'input_image' | 'output_text' }.
// String content is left alone.
export function convertMessagesToResponsesInput(messages) {
  if (!Array.isArray(messages)) return messages;

  const mapPart = (part, role) => {
  ...
  return messages.map(msg => {
    ...
    return { ...msg, content: msg.content.map(p => mapPart(p, msg.role)) };
  });
}
...
class GrokResponsesPlugin extends OpenAIVisionPlugin {
  ...
  logRequestData(data, responseData, prompt) {
    ...
    if (messages && messages.length > 1) {
      logger.info(
        `[grok responses request sent containing ${messages.length} messages]`,
      );
      let totalLength = 0;
      let totalUnits;
      ...
      logger.info(
        `[grok responses request contained ${totalLength} ${totalUnits}]`,
      );
      ...
      const { length, units } = this.getLength(content);
      logger.info(
        `[grok responses request sent containing ${length} ${units}]`,
      );
    }
    ...
    if (!stream) {
      ...
      if (typeof parsedResponse === "string") {
        ...
        logger.info(
          `[grok responses response received containing ${length} ${units}]`,
        );
      } else {
  ...
  convertSearchParametersToTools(searchParams) {
    ...
    for (const source of sources) {
      if (source.type === "web" || source.type === "news") {
        ...
        if (source.allowed_websites)
          filters.allowed_domains = source.allowed_websites.slice(0, 5);
        if (source.excluded_websites)
          filters.excluded_domains = source.excluded_websites.slice(0, 5);
        if (source.country) webSearchTool.country = source.country;
        if (source.safe_search !== undefined)
          webSearchTool.safe_search = source.safe_search;
        if (searchParams.enable_image_understanding)
      ...
      if (source.type === "x") {
        ...
        if (source.included_x_handles)
          xSearchTool.allowed_x_handles = source.included_x_handles.slice(
            0,
            10,
          );
        if (source.excluded_x_handles)
          xSearchTool.excluded_x_handles = source.excluded_x_handles.slice(
            0,
            10,
          );
        if (searchParams.from_date)
          xSearchTool.from_date = searchParams.from_date;
        if (searchParams.to_date) xSearchTool.to_date = searchParams.to_date;
        if (searchParams.enable_image_understanding)
          xSearchTool.enable_image_understanding = true;
        if (searchParams.enable_video_understanding)
  ...
  validateAndTransformTools(tools) {
    ...
    if (tools.web_search !== undefined) {
      ...
      if (webSearch.allowed_domain)
        filters.allowed_domains = webSearch.allowed_domain.slice(0, 5);
      if (webSearch.excluded_domain)
        filters.excluded_domains = webSearch.excluded_domain.slice(0, 5);
      if (webSearch.country) webSearchTool.country = webSearch.country;
      if (webSearch.safe_search !== undefined)
        webSearchTool.safe_search = webSearch.safe_search;
      if (webSearch.enable_image_understanding)
        webSearchTool.enable_image_understanding = true;
    ...
    if (tools.x_search !== undefined) {
      ...
      if (xSearch.allowed_x_handles)
        xSearchTool.allowed_x_handles = xSearch.allowed_x_handles.slice(0, 10);
      if (xSearch.excluded_x_handles)
        xSearchTool.excluded_x_handles = xSearch.excluded_x_handles.slice(
          0,
          10,
        );
      if (xSearch.from_date) xSearchTool.from_date = xSearch.from_date;
      if (xSearch.to_date) xSearchTool.to_date = xSearch.to_date;
      if (xSearch.enable_image_understanding)
        xSearchTool.enable_image_understanding = true;
      if (xSearch.enable_video_understanding)
        xSearchTool.enable_video_understanding = true;
  ...
  async getRequestParameters(text, parameters, prompt) {
    ...
    // The Responses API expects max_output_tokens, not the Chat Completions
    // max_tokens that OpenAIVisionPlugin produces.
    ...
    if (parameters.search_parameters) {
      try {
        const searchParams =
          typeof parameters.search_parameters === "string"
            ? JSON.parse(parameters.search_parameters)
            : parameters.search_parameters;

        const convertedTools =
          this.convertSearchParametersToTools(searchParams);
        if (convertedTools) {
  ...
  async execute(text, parameters, prompt, cortexRequest) {
    ...
    // The Responses API uses "input" array instead of "messages", and content
    // parts use input_text/input_image/output_text rather than text/image_url.
    if (requestParameters.messages) {
      requestParameters.input = convertMessagesToResponsesInput(requestParameters.messages);
  ...
  parseResponsesApiFormat(data) {
    ...
    const extractCitationMetadata = (url, text) => {
      ...
      try {
        ...
        if (match) {
          ...
          const timeMatch = contextBefore.match(
            /(\d{1,2}:\d{2}(?::\d{2})?(?:\s*(?:AM|PM|GMT))?)/i,
          );
          const dateMatch = contextBefore.match(
          ...
          const typeMatch = contextBefore.match(
            /\((quote|reply|repost|thread)\)/i,
          );
          if (typeMatch) {
        ...
        logger.debug(
          `[grok responses] Citation metadata extraction failed: ${e.message}`,
        );
      }
    ...
    if (citations.length > 0) {
      ...
      const enrichedCount = citations.filter(
        (c) => c.author || c.timestamp || c.postType,
      ).length;
      logger.info(
        `[grok responses] Extracted ${citations.length} citations (${enrichedCount} with rich metadata)`,
      );
      if (enrichedCount > 0) {
        const sample = citations.find((c) => c.author || c.timestamp);
        if (sample) {
          logger.debug(
            `[grok responses] Sample citation: ${JSON.stringify(sample)}`,
          );
        }
      }
    }
  ...
  processStreamEvent(event, requestProgress) {
    ...
    // xAI streams: content deltas + message_stop (citations may arrive on final event)

    const type = parsedMessage?.type;
    const delta = parsedMessage?.delta;

    // Handle different event types from xAI Responses API
          ...
          choiceDelta.tool_calls.forEach((toolCall) => {
            ...
            if (!this.toolCallsBuffer[index]) {
              ...
              if (toolCall.function?.name) {
                this.toolCallsBuffer[index].function.name +=
                  toolCall.function.name;
              }
              if (toolCall.function?.arguments) {
                this.toolCallsBuffer[index].function.arguments +=
