...
const buildMinimaxMusicAudioControls = (combinedParameters) => ({
  ...
  audio_format: normalizeEnumValue(
    combinedParameters.audio_format ?? combinedParameters.audioFormat ?? combinedParameters.outputFormat,
    ["mp3", "wav", "pcm"],
    "wav",
  ),
});
...
class ReplicateApiPlugin extends ModelPlugin {
  ...
  getRequestParameters(text, parameters, prompt) {
    ...
    switch (combinedParameters.model) {
      ...
      case "replicate-seedance-2.0": {
        ...
        const duration = Number.isInteger(combinedParameters.duration)
          && (combinedParameters.duration === -1 || (combinedParameters.duration >= 5 && combinedParameters.duration <= 15))
          ? combinedParameters.duration
          : 5;

        const firstFrameImage = normalizeImageEntry(combinedParameters.image);
        const lastFrameImage = normalizeImageEntry(combinedParameters.last_frame_image ?? combinedParameters.lastFrameImage);

        const referenceImageCandidates = [];
        ['reference_images', 'referenceImages'].forEach((field) => {
          collectImages(combinedParameters[field], referenceImageCandidates);
        });
        ...
        ['reference_videos', 'referenceVideos'].forEach((field) => {
          collectImages(combinedParameters[field], referenceVideoCandidates);
        });
        ...
        ['reference_audios', 'referenceAudios'].forEach((field) => {
          collectImages(combinedParameters[field], referenceAudioCandidates);
        });
        ...
        requestParameters = {
          input: omitUndefined({
            prompt: modelPromptText,
            duration,
            resolution: validResolutions.includes(combinedParameters.resolution) ? combinedParameters.resolution : "720p",
            aspect_ratio: validRatios.includes(combinedParameters.aspectRatio) ? combinedParameters.aspectRatio : "16:9",
            generate_audio: combinedParameters.generateAudio ?? combinedParameters.generate_audio ?? true,
            ...(Number.isInteger(combinedParameters.seed) ? { seed: combinedParameters.seed } : {}),
            ...(firstFrameImage ? { image: firstFrameImage } : {}),
            ...(firstFrameImage && lastFrameImage ? { last_frame_image: lastFrameImage } : {}),
            ...(useReferenceMode && referenceImages.length > 0 ? { reference_images: referenceImages } : {}),
            ...(useReferenceMode && referenceVideos.length > 0 ? { reference_videos: referenceVideos } : {}),
      ...
      case "replicate-grok-imagine-video": {
        const validRatios = ["auto", "16:9", "4:3", "1:1", "9:16", "3:4", "3:2", "2:3"];
        ...
        requestParameters = {
          input: video
            ...
            : {
                ...basePayload,
                duration: Math.max(1, Math.min(15, combinedParameters.duration || 5)),
                resolution: validResolutions.includes(combinedParameters.resolution) ? combinedParameters.resolution : "720p",
                aspect_ratio: validRatios.includes(combinedParameters.aspect_ratio ?? combinedParameters.aspectRatio)
                  ? (combinedParameters.aspect_ratio ?? combinedParameters.aspectRatio)
                  : "auto",
              },
        };
      ...
      case "replicate-seedream-4.5": {
        ...
        const validRatios = ["match_input_image", "1:1", "4:3", "3:4", "16:9", "9:16", "3:2", "2:3", "21:9"];
        const validSequentialModes = ["disabled", "auto"];
        const disableSafetyChecker = combinedParameters.disable_safety_checker ?? combinedParameters.disableSafetyChecker ?? false;
        ...
        const inputPayload = omitUndefined({
          prompt: modelPromptText,
          size: validSizes.includes(combinedParameters.size) ? combinedParameters.size : "2K",
          max_images: combinedParameters.numberResults || combinedParameters.maxImages || 1,
          aspect_ratio: validRatios.includes(combinedParameters.aspectRatio) ? combinedParameters.aspectRatio : "match_input_image",
          sequential_image_generation: validSequentialModes.includes(combinedParameters.sequentialImageGeneration) ? combinedParameters.sequentialImageGeneration : "disabled",
          disable_safety_checker: disableSafetyChecker,
          ...(normalizedImages.length > 0 ? { image_input: normalizedImages } : {}),
        });
      ...
      case "replicate-seedream-5-lite": {
        ...
        const inputPayload = omitUndefined({
          ...
          aspect_ratio: validRatios.includes(combinedParameters.aspectRatio) ? combinedParameters.aspectRatio : "match_input_image",
          sequential_image_generation: validSequentialModes.includes(combinedParameters.sequentialImageGeneration) ? combinedParameters.sequentialImageGeneration : "disabled",
          output_format: validOutputFormats.includes(combinedParameters.output_format ?? combinedParameters.outputFormat)
            ? (combinedParameters.output_format ?? combinedParameters.outputFormat)
            : "png",
          ...(normalizedImages.length > 0 ? { image_input: normalizedImages } : {}),
        });
      ...
      case "replicate-elevenlabs-music": {
        const musicLengthMs = combinedParameters.music_length_ms ?? combinedParameters.musicLengthMs;
        const durationSeconds = clampNumber(
          combinedParameters.duration
            ?? combinedParameters.durationSeconds
            ?? combinedParameters.length
            ?? (musicLengthMs ? Number(musicLengthMs) / 1000 : undefined),
          5,
          300,
          10,
        );
        ...
        requestParameters = {
          input: {
            ...
            force_instrumental: combinedParameters.force_instrumental ?? combinedParameters.forceInstrumental ?? true,
          },
        };
        break;
      }
      case "replicate-minimax-music-26": {
        requestParameters = {
          input: omitUndefined({
            ...
            is_instrumental: combinedParameters.is_instrumental ?? combinedParameters.isInstrumental ?? false,
            lyrics_optimizer: combinedParameters.lyrics_optimizer ?? combinedParameters.lyricsOptimizer ?? false,
          }),
        };
        break;
      }
      case "replicate-minimax-music-cover": {
        ...
        if (!audioUrl) {
          throw new Error("MiniMax Music Cover requires a selected audio item or audio_url");
        }
      ...
      case "replicate-qwen3-tts": {
        ...
        if (mode === "voice_clone" && !referenceAudio) {
          throw new Error("Qwen3 TTS voice_clone mode requires reference_audio or a selected audio item");
        }
        if (mode === "voice_design" && !voiceDescription) {
          throw new Error("Qwen3 TTS voice_design mode requires voice_description");
        }
      ...
      case "replicate-elevenlabs-v3": {
        ...
        const similarityBoost = clampNumber(
          combinedParameters.similarity_boost ?? combinedParameters.similarityBoost,
          0,
          1,
          0.75,
        );
        const style = clampNumber(
      ...
      case "replicate-minimax-speech-2.8-hd": {
        ...
        const voiceId = getFirstDefined(
          selectedVoiceId === "__custom" ? customVoiceId : selectedVoiceId,
          "English_Wiselady",
        );
        ...
        requestParameters = {
          input: omitUndefined({
            ...
            volume: clampNumber(combinedParameters.volume, 0, 10, 1),
            pitch: Math.round(clampNumber(combinedParameters.pitch, -12, 12, 0)),
            emotion: normalizeEnumValue(
              combinedParameters.emotion,
              MINIMAX_TTS_EMOTIONS,
              "auto",
            ),
            english_normalization: combinedParameters.english_normalization
            ...
            audio_format: normalizeEnumValue(
              combinedParameters.audio_format ?? combinedParameters.audioFormat,
            ...
            subtitle_enable: combinedParameters.subtitle_enable
              ?? combinedParameters.subtitleEnable
              ?? false,
            language_boost: normalizeEnumValue(
              combinedParameters.language_boost ?? combinedParameters.languageBoost,
  ...
  getPredictionFailureMessage(status, errorMessage) {
    ...
    const modelName = this.modelName || this.model?.name || this.promptParameters?.model;
  ...
  async execute(text, parameters, prompt, cortexRequest) {
    ...
    const isLongRunningMedia = ['audio', 'video', 'tts'].includes(this.model?.metadata?.category);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        if (status === "succeeded") {
          ...
          throw new Error(this.getPredictionFailureMessage(status, pollResponse.data?.error));
        }
  ...
  collectOutputUrls(value, urls = [], path = []) {
    ...
    if (Array.isArray(value)) {
      value.forEach((item, index) => this.collectOutputUrls(item, urls, [...path, String(index)]));
      return urls;
    }
  ...
  parseResponse(data) {
    ...
    for (const outputItem of outputItems) {
      ...
      if (artifactType === 'audio' && mimeType === 'application/octet-stream') {
      ...
      artifacts.push({
        ...
        url: outputItem.url,
        mimeType,
        ...(outputItem.key && outputItem.key !== 'output' ? { key: outputItem.key } : {}),
      });
    }
  ...
  logRequestData(data, responseData, prompt) {
    ...
    const responseText = typeof parsedResponse === 'string'
      ? parsedResponse
      : parsedResponse?.output_text || '';
    const responseLength = this.getLength(responseText);
    logger.info(`[Replicate response received containing ${responseLength.length} ${responseLength.units}]`);
