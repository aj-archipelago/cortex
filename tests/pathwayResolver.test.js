...
test('processRequest falls back to serial processing when useParallelPromptProcessing is false', async (t) => {
  ...
  // Ensure parallel processing is disabled by setting up a completely new pathway
  resolver.pathway = {
    ...resolver.pathway,
  ...
  // Override processRequest to test the condition logic directly
  const originalProcessRequest = resolver.processRequest.bind(resolver);
  resolver.processRequest = async function(params) {
    // Simulate the condition check from the real processRequest
    if (this.pathway.useParallelPromptProcessing && this.prompts.length > 1) {
      return ['parallel result 1', 'parallel result 2']; // This should NOT happen
    } else {
      return 'serial result'; // This SHOULD happen
    }
  };
  
  const result = await resolver.processRequest({ text: 'test input' });
  
  // Should get serial result since useParallelPromptProcessing is false
  t.is(result, 'serial result');
});
